...
config.updateScreenShareConfig({
  ...
  mode: 'video',                    // 'presentation' | 'video' — задаёт maxFrameRate и contentHint
  maxWidth: 1920,                   // ограничение ширины
  maxHeight: 1080,                  // ограничение высоты
  maxFrameRate: 15,                 // ограничение FPS (перекрывает значение из mode)
  contentHint: 'detail',           // 'motion' | 'detail' | 'text' | '' (перекрывает значение из mode)
})

// mode задаёт оптимальные maxFrameRate и contentHint:
// 'presentation' — 5 FPS, contentHint='text' (слайды, документы — лёгкий для CPU)
// 'video'        — 30 FPS, contentHint='motion' (фильмы, игры — плавный)
// Явно заданные maxFrameRate/contentHint перекрывают значения из mode.
...
// maxDuration — в секундах (0 = без ограничений)
// maxFileSize — в MB (0 = без ограничений)
config.setRecordingLimits(60, 500)   // 60 с, 500 MB
...
interface ScreenShareConfiguration {
  ...
  maxFrameRate?: number         // ограничение FPS (если не задано — из mode)
...
interface RecordingConfiguration {
  ...
  maxDuration: number           // максимальная длительность в секундах (0 = без ограничений)
